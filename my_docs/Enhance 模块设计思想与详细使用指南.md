...
Enhance 模块大量使用 **Hook 机制** 实现可扩展性：

```kotlin
// Prompt Hook: 在对话历史准备的各个阶段插入自定义逻辑
PromptHookRegistry.dispatchPromptHistoryHooks(context)      // 历史准备前/后
PromptHookRegistry.dispatchPromptInputHooks(context)        // 输入处理前/后
PromptHookRegistry.dispatchSystemPromptComposeHooks(context) // 系统提示词组装
PromptHookRegistry.dispatchToolPromptComposeHooks(context)   // 工具提示词组装

// Summary Hook: 在总结生成的各个阶段插入自定义逻辑
SummaryHookRegistry.dispatchSummaryGenerateHooks(context)   // 总结生成各阶段
```
